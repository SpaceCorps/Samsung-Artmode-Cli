//! Non-secret account metadata in a readable YAML file. The auth token itself never lands here -
...
pub struct AccountConfig {
    /// IP address or hostname of the Samsung The Frame TV
    pub host: String,
    /// MAC address of the TV (optional, useful for Wake-on-LAN)
    #[serde(skip_serializing_if = "String::is_empty")]
    pub mac: String,
    /// TV model name (e.g. QN55LS03BAFXZA)
    #[serde(skip_serializing_if = "String::is_empty")]
    pub model: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("SAMSUNG_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("samsung-artmode-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("samsung-artmode-cli")
        } else {
            std::env::var_os("XDG_CONFIG_HOME")
                .filter(|v| !v.is_empty())
                .map(PathBuf::from)
                .unwrap_or_else(|| home.join(".config"))
                .join("samsung-artmode-cli")
        }
    };
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: samsung-artmode accounts add <name> --host <ip>", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another samsung-artmode process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn parses_config() {
        ...
        let (name, acct) = c.find("livingroom").unwrap();
        assert_eq!(name, "LivingRoom");
        assert_eq!(acct.host, "192.168.1.100");
    }
}

//! `samsung-artmode` - a native CLI for controlling Samsung The Frame TV Art Mode.
...
fn main() -> ExitCode {
    // Pre-scan raw args for --json so errors can be formatted as JSON if requested
    ...
    match commands::run(cli.command, cli.account, cli.host, cli.token, cli.verbose) {
        Ok(()) => ExitCode::SUCCESS,

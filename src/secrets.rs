//! Where TV auth tokens live: the OS keystore, one entry per account under `account:{name}`.
//!
//! DPAPI on Windows, the Keychain on macOS, libsecret on Linux. When none is available the tool
//! refuses to start rather than silently writing a file - `SAMSUNG_ALLOW_PLAINTEXT_STORE=1` is
...
const SERVICE: &str = "samsung-artmode-cli";
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("SAMSUNG_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "SAMSUNG_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset SAMSUNG_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
    ...
    fallback(
        "libsecret is not installed, so there is no OS keystore to hold your TV auth tokens. \
         Install it with: sudo apt install libsecret-tools  (or the equivalent for your distro).",
    )
}

fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("SAMSUNG_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1")
    ...
    Err(Error::other("No secure credential store is available on this machine.")
        .detail(reason)
        .fix("Install a keystore, or set SAMSUNG_ALLOW_PLAINTEXT_STORE=1 to store TV tokens in a 0600 file instead."))
}
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: TV tokens are stored unencrypted in {} (SAMSUNG_ALLOW_PLAINTEXT_STORE=1).",
...
mod dpapi {
    ...
    pub fn protect(plain: &[u8]) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("DPAPI could not encrypt the TV auth tokens.")
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored TV auth tokens could not be decrypted.")
                .detail(format!(
                    "DPAPI failed: {}. This happens when the file was written by a different Windows user or on a different machine.",
                    std::io::Error::last_os_error()
                ))
                .fix(format!("Delete {} and run: samsung-artmode pair <ip>", path.display())));
        }
        Ok(take(out))
    }
}
